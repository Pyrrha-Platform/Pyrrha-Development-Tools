...
class WorkspaceChecker {
  ...
  checkRepository(repoPath, repoName) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      // Check for Prettier
      if (!packageJson.devDependencies?.prettier && !fs.existsSync(path.join(repoPath, '.prettierrc.js'))) {
      ...
      if (repoName === 'Pyrrha-Dashboard') {
        ...
        if (!fs.existsSync(dashboardPath)) {
          this.addIssue(repoName, 'Expected pyrrha-dashboard subdirectory not found', 'error');
        } else {
          // Check Flask API
          ...
          if (fs.existsSync(flaskApiPath)) {
            ...
            if (!fs.existsSync(venvPath)) {
              this.addIssue(repoName, 'Flask API virtual environment not found', 'warn');
            }
          }
        }
      }
    ...
    if (fs.existsSync(requirementsPath)) {
      ...
      if (!fs.existsSync(pyprojectPath)) {
        this.addIssue(repoName, 'No pyproject.toml found for Python configuration', 'info');
      }
    }
    
    // Check for C/C++/Arduino projects
    const cppFiles = fs.readdirSync(repoPath, { recursive: true })
      .filter(f => /\.(c|cpp|h|hpp|ino)$/.test(f));
    if (cppFiles.length > 0) {
      this.log(`  🔩 C/C++/Arduino project detected (${cppFiles.length} files)`, 'success');
      ...
      if (!fs.existsSync(clangFormatPath)) {
        this.addIssue(repoName, 'No .clang-format found for C/C++ formatting', 'info');
      }
    }
    
    // Check for Dockerfiles
    const dockerfiles = fs.readdirSync(repoPath).filter(f => f.startsWith('Dockerfile'));
    if (dockerfiles.length > 0) {
      this.log(`  🐳 Docker configuration found (${dockerfiles.length} files)`, 'success');
    }
  ...
  async run() {
    ...
    // Recommendations
    this.addRecommendation('Run "npm run setup:hooks" to install Git pre-commit hooks');
    this.addRecommendation('Run "npm run lint:all" to test linting across all repositories');
    this.addRecommendation('Consider adding CI/CD integration with centralized linting');
